...
public class CustomerView extends JPanel 
{
	...
	public void searchByTel(){
		...
		try {
			...
			tfCustTel.setText(vo.getpNo());
			tfCustTelAid.setText(vo.getS_Pno());
	...
	/*
	public void searchByPno(){

		// 1. 입력한 전화번호 얻어오기
		String name = bCustNameSearch.getText();
		// 2. Model의 전화번호 검색메소드 selectByTel()  호출
		try {
			CustomerVO vo = model.selectByTel(tel);
			// 3. 2번의 넘겨받은 Customer의 각각의 값을 화면 텍스트 필드 지정
...
*/
	...
	public void connectDB(){		//  CustomerDaoImpl 패키지에서 접속 연결 잘됐는지 확인시켜줌
		try { 			
